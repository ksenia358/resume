...
import { getEducation } from '../api/resume'
...
export function Education() {
  ...
  return (
    <Flex vertical gap="middle">
      {groups.map((group) => (
        <InstitutionContent key={group.institution} group={group} />
      ))}
    </Flex>
  )
}
...
function InstitutionContent({ group }: { group: InstitutionGroup }) {
  ...
  return (
    <div>
      <Text strong>
        {group.url ? (
          <a href={group.url} target="_blank" rel="noreferrer" style={{ color: 'inherit' }}>
            {group.institution}
          </a>
        ) : (
          group.institution
      ...
      {group.degrees.map((degree) => (
        <div key={degree.id} style={{ marginTop: 4 }}>
